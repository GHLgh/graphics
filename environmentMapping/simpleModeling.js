...
function skybox(vertexArray, normalArray){
    var numT = 0;
    var brb = vec4.fromValues(-1.0,-1.0,-1.0,0);
    var blb = vec4.fromValues( 1.0,-1.0,-1.0,0);
    var blt = vec4.fromValues(-1.0, 1.0,-1.0,0);
    var brt = vec4.fromValues( 1.0, 1.0,-1.0,0);
    var frb = vec4.fromValues(-1.0,-1.0, 1.0,0);
    var flb = vec4.fromValues( 1.0,-1.0, 1.0,0);
    var flt = vec4.fromValues(-1.0, 1.0, 1.0,0);
    var frt = vec4.fromValues( 1.0, 1.0, 1.0,0);
    
    var normalBack = vec4.fromValues(0.0,0.0,1.0,0);
    
    pushVertex(blt, vertexArray);
    pushVertex(brb, vertexArray);
    pushVertex(brt, vertexArray);

    pushVertex(blt, vertexArray);
    pushVertex(blb, vertexArray);
    pushVertex(brb, vertexArray);

    for(i = 0; i < 6; i++)
        pushVertex(normalBack, normalArray);
    
    numT += 2;
    /*
    pushVertex(, vertexArray);
    pushVertex(, vertexArray);
    pushVertex(, vertexArray);
